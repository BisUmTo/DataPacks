# redcraft2:src/horsestats/brackets/1

tellraw @p ["\n",{"selector":"@s","bold":true}]

# Vita
execute store result score @s REDHRSstt run data get entity @s Attributes[0].Base
tellraw @p [{"text":"∙ Vita: "},{"score":{"name":"@s","objective":"REDHRSstt"},"hoverEvent":{"action":"show_text","value":"15 – 30"}}]

# Velocità
execute store result score @s REDHRSstt run data get entity @s Attributes[2].Base 10000
...
# Salto
execute store result score @s REDHRSstt run data get entity @s Attributes[6].Base 1000
